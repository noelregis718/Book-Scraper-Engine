...
async def process_book(context, scraper, idx, title, author, df, semaphore):
    async with semaphore:
        ...
        print(f"[{idx}] Scraping '{safe_title}' by '{author}'...")
        try:
            ...
            if data:
                ...
                print(f"[{idx}] Done parsing '{safe_title}'.")
            else:
...
async def run_scrape():
    ...
    # Make sure we have the required columns initialized
    required_cols = [
    ...
    async with async_playwright() as p:
        ...
        for idx in range(len(df)):
            ...
            # Skip empty titles or authors
            if not title or title.lower() == 'nan' or not author or author.lower() == 'nan':
                continue
