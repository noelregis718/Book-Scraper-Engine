...
async def scrape_one_book(index, title, author_for_search, df, excel_path, scraper, shared_context, semaphore):
    async with semaphore:
        ...
        try:
            ...
            # Since we only searched by author, we should update the series/book name with what we actually found
            title_el = await page.query_selector('h1[data-testid="bookTitle"]')
            if title_el:
                new_title = clean_text(await title_el.inner_text())
                if new_title:
                    df.at[index, "Name of Series"] = new_title
...
async def scrape_updated_v2_aggressive(excel_path):
    ...
    async with async_playwright() as p:
        ...
        print("Login done. Starting aggressive scrape...\n")
        ...
        for index, row in df.iterrows():
            ...
            if author in ["nan", "None", "", "Unknown – pre-pub / unannounced", "TLA Client"]:
                author_for_search = ""
            else:
        ...
        if not tasks:
            print("No missing books found to scrape!")
...
if __name__ == "__main__":
    ...
    target = os.path.join(base, "books_scraped_updated_v2.xlsx")
    asyncio.run(scrape_updated_v2_aggressive(target))
