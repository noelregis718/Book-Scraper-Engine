...
KEYWORD_MAP = [
    ...
    ("Urban / Contemporary Fantasy Romance", ["urban fantasy", "modern day", "contemporary fantasy", "hidden world", "secret magic", "real world", "city magic", "supernatural city", "romance"])
]
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        existing_genres = str(row.get(subgenre_col, '')).strip() # Currently holds raw Goodreads genres
        
        if title.lower() == 'nan' or not title:
            continue
            
        combined_text = synopsis + " " + title + " " + existing_genres
        ...
        if subgenre_result is not None:
            ...
            df.at[idx, subgenre_col] = "" # Clear the raw genres if it's not romantasy
