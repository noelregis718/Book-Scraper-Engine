...
async def scrape_one_book(index, title, author_for_search, df, excel_path, context, semaphore):
    async with semaphore:
        # Instead of a new context (which opens a new window), we just open a new page (tab) in the main context
...
async def scrape_handspun(excel_path, limit=300):
    ...
    # The semaphore ensures only 10 tabs are open at a time
    ...
    async with async_playwright() as p:
        ...
        # Create one shared context that will act as the single browser window
        ...
        for index, row in df.iterrows():
            ...
            author_for_search = author if author not in ["nan", "", "Unknown – pre-pub / unannounced", "TLA Client"] else ""
...
if __name__ == "__main__":
    ...
    asyncio.run(scrape_handspun(target, limit=300))
