...
async def run_scrape():
    ...
    async with async_playwright() as p:
        ...
        print("--- Finding the #1 top book for each author to add if missing ---")
        for author in authors:
            safe_author = author.encode('ascii', 'ignore').decode('ascii')
            print(f"Checking author: {safe_author}")
            
            # Specifically requested only the FIRST book
            ...
            for book in top_books:
                ...
                # Break here to ensure we only process the very first book returned
                break
