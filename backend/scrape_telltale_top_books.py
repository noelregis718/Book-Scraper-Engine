...
TRACKING_FILE = r"E:\Internship\PocketFM\backend\telltale_authors_processed.txt"
...
async def process_new_book(context, scraper, title, author, semaphore):
    async with semaphore:
        ...
        row_data = {
            ...
            "Name of agent in the main folder": "Tell-Tale Publishing"
        }
...
async def run_scrape():
    ...
    print(f"Found {len(authors)} remaining authors. Launching aggressive Top Books scraping for ALL of them!")
    ...
    async with async_playwright() as p:
        ...
        print(f"--- Finding the top 3 books for {len(authors)} authors ---")
        for author in authors:
            ...
            # Mark author as processed (progressive save)
        ...
        if not new_books_to_scrape:
            print("\nNo new books to scrape for these authors. They are already in the sheet!")
        else:
            print(f"\n--- Scraping details for {len(new_books_to_scrape)} new books concurrently ---")
