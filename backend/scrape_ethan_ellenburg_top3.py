...
MAX_CONCURRENT_AUTHORS = 2  # To not overload with tabs
...
async def run_scrape():
    ...
    unique_authors = df["Author Name"].dropna().unique().tolist()
    ...
    async with async_playwright() as p:
        ...
        tasks = [process_author(author, context, semaphore) for author in unique_authors]
    ...
    print(f"Saving {len(new_df)} total rows to {EXCEL_FILE}...")
