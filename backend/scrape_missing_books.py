...
async def scrape_book(index, title, author, df, context, semaphore):
    async with semaphore:
        if not title or title in ["nan", "None", "", "N/A"]:
            return
        ...
        try:
            ...
            print(f"[{index}] Searching for: {query}")
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for index, row in df.iterrows():
            ...
            # If the row is missing critical data, we add it to the scrape list
            if not synopsis or synopsis in ["nan", "None", "N/A"] or rating in ["", "nan", "None", "N/A"]:
                if title and title not in ["nan", "None", "N/A"]:
                    tasks.append(scrape_book(index, title, author, df, context, semaphore))
        
        if tasks:
            print(f"Found {len(tasks)} missing books to scrape concurrently...")
            await asyncio.gather(*tasks)
        else:
            print("No missing books found! All rows are already full.")
