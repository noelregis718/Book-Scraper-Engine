...
async def aggressive_search(context, title, author, semaphore):
    async with semaphore:
        ...
        try:
            ...
            query = f'"{safe_title}" {safe_author} site:goodreads.com/book/show/'
...
async def run_fix():
    ...
    async with async_playwright() as p:
        ...
        for idx, row in df.iterrows():
            if idx < 106:
                continue # Skip the first 106 rows
            ...
            if pd.isna(link) or link == '' or str(link).strip() == 'N/A' or str(link).strip() == 'nan':
                ...
                if pd.isna(title) or not str(title).strip() or str(title).lower() == 'nan':
                    continue
                if pd.isna(author) or not str(author).strip() or str(author).lower() == 'nan':
                    continue
