"""
Mushens Entertainment GoodReads Scraper
=======================================
Pattern: ONE browser window, ONE shared context, tabs open inside it.
Semaphore limits to 10 concurrent tabs at a time.

Maps scraped Goodreads data into the 11-column sheet:
...
Publisher (col C) stays "Mushens Entertainment", Agent (col K) stays "Juliet Mushens",
Name of Series + Author Name are NEVER overwritten.
"""
...
CONCURRENCY = 10  # 10 tabs inside one browser window
...
async def process_row(context, scraper, df, idx, semaphore, excel_path, file_lock):
    async with semaphore:
        ...
        # Incremental save after every book
...
async def scrape_mushens(excel_path, book_limit=200):
    ...
    async with async_playwright() as p:
        ...
        print(f"[System] Login done. Starting scrape with {CONCURRENCY} tabs...\n")
    ...
    # Re-apply the Mushens 11-column white-row styling (preserves all scraped values)
    print("Reapplying Mushens 11-column styling...")
...
if __name__ == "__main__":
    ...
    asyncio.run(scrape_mushens(excel_path, book_limit=200))
