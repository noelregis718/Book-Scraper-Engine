...
async def scrape_one_book(index, title, author_for_search, df, excel_path, scraper, browser, storage_state, semaphore):
    async with semaphore:
        # Each task gets its OWN isolated context using saved login cookies
        ...
        try:
            ...
            # Check if Goodreads auto-redirected straight to a book page (1 result case)
            current_url = page.url
            if "/book/show/" in current_url:
                book_url = current_url
                print(f"[{index+1}] Auto-redirected to: {book_url}")
            else:
                # Multiple results - grab first link
                try:
                    ...
                    print(f"[{index+1}] Found: {book_url}")
                except Exception:
...
async def scrape_tobias(excel_path, limit=10):
    ...
    # Cast all target columns to object type to avoid FutureWarning
    ...
    async with async_playwright() as p:
        ...
        for index, row in df.iterrows():
            ...
            if current_link and current_link not in ["nan", "", "N/A"]:
                # print(f"[{index+1}] Skipping '{title}' — already has link.")
                continue
...
if __name__ == "__main__":
    ...
    asyncio.run(scrape_tobias(target, limit=40))
