...
async def scrape_romaunce_aggressive(excel_path):
    ...
    async with async_playwright() as p:
        ...
        for index, row in df.iterrows():
            ...
            # Scrape if link is empty or NaN
            if link not in ["N/A", "nan", "", "None"] and pd.notna(row.get("GoodReads series link")):
                # The user wants to scrape all of them, but let's only skip if it's already filled properly
                pass
    ...
    # Skip applying JRA styling since we already have our own styling logic from format_romaunce_final.py
    # or we can apply it. Let's just open the excel file.
...
if __name__ == "__main__":
    ...
    asyncio.run(scrape_romaunce_aggressive(target))
