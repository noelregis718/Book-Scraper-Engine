...
async def process_row(context, scraper, idx, title, author, df, semaphore):
    async with semaphore:
        ...
        try:
            print(f"[{idx+1}] Aggressively Searching: '{safe_title}' by {safe_author}...", flush=True)
            ...
            if data:
                ...
                print(f"[{idx+1}] Successfully scraped '{safe_title}'. Romantasy: {df.at[idx, 'Romantasy = Yes or No?']}", flush=True)
            else:
                print(f"[{idx+1}] Not Found '{safe_title}'.", flush=True)
            ...
            print(f"[{idx+1}] Error scraping '{safe_title}': {e}", flush=True)
...
async def run_aggressive_scrape():
    ...
    async with async_playwright() as p:
        ...
        print(f"--- Queuing first {NUM_TO_PROCESS} books for aggressive scraping ---", flush=True)
