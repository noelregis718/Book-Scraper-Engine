...
async def process_missing_book(context, scraper, idx, title, author, df, semaphore):
    async with semaphore:
        safe_title = str(title).encode('ascii', 'ignore').decode('ascii')
        print(f"[{idx}] Retrying '{safe_title}'...")
        try:
            ...
            if data:
                ...
                print(f"[{idx}] Successfully recovered details for '{safe_title}'!")
            else:
                print(f"[{idx}] Details STILL not found for '{safe_title}'.")
...
async def run_scrape():
    ...
    async with async_playwright() as p:
        ...
        for idx in range(len(df)):
            ...
            # If link is missing/empty/nan/N/A
            if link == '' or link.lower() == 'nan' or link == 'N/A' or link == 'None':
                missing_count += 1
                tasks.append(process_missing_book(context, scraper, idx, title, author, df, semaphore))
            
        print(f"Found {missing_count} books missing links. Starting recovery...")
    ...
    # We should run the final mapping again just in case the newly scraped synopses changed the Romantasy status
    print("Re-running final subgenre mapping to ensure new synopses are classified...")
    try:
        import classify_writershouse_final
        classify_writershouse_final.main()
    except Exception as e:
