    ...
    try:
        with sync_playwright() as p:
            ...
            for search_attempt in range(5):
                try:
                    print(f"[Z-Library] [{task.title}] Searching... (Attempt {search_attempt+1}/5)")
                    ...
                    if not top_links:
                        if search_attempt < 4:
                            ...
                            print(f"[Z-Library] [{task.title}] Gave up after 5 failed search attempts.")
                            browser.close()
                            return
                    ...
                    if download_successful:
                        break # Break out of the 5-attempt loop
                    else:
                        if search_attempt < 4:
                            ...
                            task.status = "failed"
                            task.error_message = "All top matches failed to download after 5 attempts."
                            print(f"[Z-Library] [{task.title}] Gave up after 5 attempts.")
                            break
                except Exception as loop_err:
                    print(f"[Z-Library] [{task.title}] Attempt {search_attempt+1} encountered an error: {loop_err}")
                    if search_attempt < 4:
                        print(f"[Z-Library] [{task.title}] Retrying...")
                        page.wait_for_timeout(5000)
                        continue
                    else:
                        task.status = "failed"
                        task.error_message = f"Failed after 5 attempts. Last error: {loop_err}"
                        print(f"[Z-Library] [{task.title}] Gave up after 5 attempts.")
                        break
