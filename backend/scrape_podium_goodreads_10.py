    ...
    async with semaphore:
        ...
        if data:
            ...
            async with excel_lock:
                ...
                print_flush(f"  [Task {i}] Saved safely.")
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        tasks = []
        # Process remaining books concurrently
        for i in range(8780, len(df)):
            row = df.iloc[i]
    ...
    print_flush("\n[Success] Processed books concurrently!")
    
    try:
        from style_podium import style_excel
        style_excel()
    except Exception as e:
        print_flush(f"[Style Warning] skipped: {e}")
