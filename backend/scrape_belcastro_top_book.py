...
async def process_new_book(context, scraper, title, author, semaphore):
    async with semaphore:
        ...
        row_data = {
            ...
            "Name of agent": "Sharon Belcastro"
        }
...
async def run_scrape():
    ...
    async with async_playwright() as p:
        ...
        if not new_books_to_scrape:
            print("No new books to scrape. The top books are already in the sheet!")
        else:
            print(f"--- Scraping {len(new_books_to_scrape)} new top books ---")
