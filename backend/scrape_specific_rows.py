...
# Excel row 78 is Pandas index 76
# Excel row 97 is Pandas index 95
START_INDEX = 76
END_INDEX = 95
...
async def process_book(scraper, context, idx, title, author, df):
    ...
    if found_url:
        print(f"  [Aggressive] Found URL on Google: {found_url}")
    else:
        print(f"  [Aggressive] No Google URL found. Will let normal scraper try.")
    ...
    if details:
        ...
        print(f"  [Failed] Could not scrape details for '{title}'")
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        print(f"\nFound {len(tasks_to_process)} books in the specified range (Row 78 to 97).")
