...
url = 'https://harpercollins.co.uk/collections/romantasy'
headers = {
...
try:
    ...
    with open('harpercollins_dump.html', 'w', encoding='utf-8') as f:
        f.write(soup.prettify())
    print("Dumped to harpercollins_dump.html")
    ...
    authors = soup.find_all(class_=lambda x: x and 'author' in x.lower())
    print(f"Found {len(authors)} potential authors.")
    if authors:
        for a in authors[:5]:
            print(f"Author: {a.text.strip()}")
