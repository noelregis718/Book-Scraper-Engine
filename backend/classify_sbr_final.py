...
def classify_subgenre(text, genre):
    ...
    if genre == "Romantasy":
        ...
        for subgenre, keywords in KEYWORD_MAP_CT:
            if any(k in text for k in keywords):
                return subgenre
        return "Psychological / Domestic Thriller"
...
def main():
    ...
    # First, quickly wipe out any existing "Unknown", "Needs Mapping", "N/A"
    df['Genre'] = df['Genre'].replace(['Unknown', 'N/A', 'Needs Mapping'], '')
    df['Sub-Genre'] = df['Sub-Genre'].replace(['Unknown', 'N/A', 'Needs Mapping'], '')
    ...
    for idx, row in df.iterrows():
        ...
        if not current_genre or current_genre.lower() == 'nan':
