...
async def process_row(context, scraper, df, idx, semaphore, excel_path, file_lock):
    async with semaphore:
        ...
        # Deliberately ignore author name as requested
        author = ""

        if title.lower() in ["", "nan"]:
            return

        print(f"  [Searching] '{title}' (Author ignored)")
...
async def scrape_stonesong_books(excel_path, limit=500):
...
if __name__ == "__main__":
    ...
    asyncio.run(scrape_stonesong_books(excel_path, limit=500))
