...
async def process_row(context, scraper, df, idx, semaphore, excel_path, file_lock):
    async with semaphore:
        ...
        if title.lower() in ["", "nan"] and author.lower() in ["", "nan"]:
            return

        print(f"  [Searching] '{title}' by {author}")

        try:
            # Scrape Goodreads data - this natively grabs the first search result aggressively
...
async def scrape_corvisiero(excel_path, limit=500):
    ...
    for idx, row in df.iterrows():
        ...
        if (not title or title.lower() in ["", "nan"]) and (not author or author.lower() in ["", "nan"]):
            continue
...
if __name__ == "__main__":
    ...
    excel_path = os.path.join(base, "corvisiero_merged.xlsx")
    # Processing all rows aggressively
    asyncio.run(scrape_corvisiero(excel_path, limit=500))
