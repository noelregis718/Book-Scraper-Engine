...
async def main():
    ...
    # We only want to scrape the new authors added in the previous step
    # Their 'Name of Series' was set to 'AUTHORS'
    new_author_rows = df[df['Name of Series'] == 'AUTHORS']
    valid_authors = new_author_rows['Author Name'].dropna().unique().tolist()
            
    print(f"Found exactly {len(valid_authors)} BRAND NEW authors to aggressively scrape.")
    
    if not valid_authors:
        print("No new authors to process. Exiting.")
        return
    ...
    async with async_playwright() as p:
        ...
        print(f"Launching direct scraper for {len(valid_authors)} authors...")
    ...
    if all_new_rows:
        ...
        # Drop the placeholder 'AUTHORS' rows since we have their books now, to keep it clean
        combined_df = combined_df[combined_df['Name of Series'] != 'AUTHORS']
