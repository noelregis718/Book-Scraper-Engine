...
EXCEL_FILE = r"E:\Internship\PocketFM\andrea_brown_books_formatted.xlsx"
MAX_CONCURRENT = 5
...
async def run_scrape():
    ...
    async with async_playwright() as p:
        ...
        print("--- Finding the first new book for each author ---")
        for author in authors:
            safe_author = author.encode('ascii', 'ignore').decode('ascii')
            print(f"Checking author: {safe_author}")
            # Get up to 3 books in case the first few are duplicates
            ...
            # Find the FIRST book that is not already in the sheet
            added = False
            for book in top_books:
                ...
                if not exists:
                    ...
                    added = True
                    break # We only want ONE book per author
                else:
                    print(f"  [Skipping] '{found_title.encode('ascii', 'ignore').decode('ascii')}' - Already in sheet, trying next...")
            
            if not added:
                print(f"  All top 3 books for {safe_author} were already in the sheet.")
