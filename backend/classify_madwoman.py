...
def main():
    ...
    for idx, row in df.iterrows():
        # Skip the last 2 rows if they were manually set
        ...
        # Don't touch if already explicitly "No" and sub-genre is mapped by user (like the llama book)
        if current_yes_no == 'No' and current_sub and current_sub != 'nan' and current_sub != 'N/A':
            continue
        ...
        if gr_romantasy == 'Yes' or subgenre_result is not None:
            df.at[idx, romantasy_col] = "Yes"
            # If we couldn't classify it but we know it's romantasy, default to High Fantasy or leave generic?
            # We'll map to the best fit or "High Fantasy Court Adventure" as fallback if missing
            df.at[idx, subgenre_col] = subgenre_result if subgenre_result else "High Fantasy Court Adventure"
            yes_count += 1
        else:
            # Not romantasy and no keywords found
    ...
    os.system(r'powershell -Command "Copy-Item e:\Internship\PocketFM\madwoman_literary_scraped_books.xlsx -Destination C:\Users\noelr\Downloads\madwoman_literary_scraped_books.xlsx -Force"')
